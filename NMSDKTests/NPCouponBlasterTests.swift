...
class NPCouponBlasterTests: XCTestCase {
    ...
    func testIndexCoupons() {
        ...
        SDKDelegate.sdkDidSync = { (errors) in
            ...
            NearSDK.downloadProcessedRecipes { (success, recipes, contents, polls) in
                NearSDK.downloadCoupons { (success) in
                    let coupons = NearSDK.coupons
                    XCTAssertEqual(coupons.count, 1)
                    
                    guard let firstCoupon = coupons.first else {
                        XCTFail("nil coupon")
                        return
                    }
                    
                    let coupon = NearSDK.coupon(withSerialNumber: firstCoupon.serialNumber)
